 ...
 * @projectName: test1
 * @package: com.axing.practice27IO
 * @className: Test25HuToolCrawling
 * @author: Axing
 * @description: TODO
 * @date: 2024/5/9 下午9:15
 * @version: 1.0
 */
public class Test25HuToolCrawling {
    public static void main(String[] args) throws IOException {
        ...
        // 从链接中获取所有字符（未筛选
        String familyNameTemp = HttpUtil.get(familyNameNet);//赵钱孙李，周吴郑王。
        String boyNameTemp = HttpUtil.get(boyNameNet);//1.昊玉、新梁、纬博、君临、瑾平
        String girlNameTemp = HttpUtil.get(girlNameNet);//【怡雯】【蕾雅】【雪娟】

        // 将字符进行初步筛选
        ArrayList<String> familyNameTempList = (ArrayList<String>) ReUtil.findAll("([\\u4E00-\\u9FA5]{4})(，|。)", familyNameTemp, 1);

        // 将字符进行最后的筛选
        ArrayList<String> boyNameList = (ArrayList<String>) ReUtil.findAll("\\d\\.[\\u4E00-\\u9FA5]{2}(、[\\u4E00-\\u9FA5]{2}){4}", boyNameTemp, 0);
        boyNameList = (ArrayList<String>) ReUtil.findAll("((\\d\\.)|(、))([\\u4E00-\\u9FA5]{2})", boyNameList.toString(), 4);
        ArrayList<String> girlNameList = (ArrayList<String>) ReUtil.findAll("(【)([\\u4E00-\\u9FA5]{2})(】)", girlNameTemp, 2);
